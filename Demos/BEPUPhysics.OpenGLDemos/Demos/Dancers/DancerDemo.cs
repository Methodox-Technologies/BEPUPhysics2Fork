...
/// </summary>
public class DancerDemo : DemoBase
{
        ...
        int instanceId, Simulation simulation, CollidableProperty<ClothCollisionFilter> filters)
    {
        ...
        float armHoleRadius = 0.095f;
        float armHoleRadiusSquared = armHoleRadius * armHoleRadius;
        float halfWidth = widthInNodes * spacing / 2;
        ...
        for (int rowIndex = 0; rowIndex < widthInNodes; ++rowIndex)
        {
            for (int columnIndex = 0; columnIndex < widthInNodes; ++columnIndex)
            {
                ...
                float distanceSquared0 = Vector2.DistanceSquared(horizontalPosition, armHoleCenter);
                float distanceSquared1 = Vector2.DistanceSquared(horizontalPosition, -armHoleCenter);
                float centerDistanceSquared = horizontalPosition.LengthSquared();
    ...
    static void TailorDress(Simulation simulation, CollidableProperty<ClothCollisionFilter> filters, DancerBodyHandles bodyHandles, int dancerIndex, int dancerGridWidth, float levelOfDetail)
    {
        ...
        float targetDressDiameter = 2.6f;
        int fullDetailWidthInBodies = 29;
        ...
        int widthInBodies = (int)MathF.Ceiling(fullDetailWidthInBodies / scale);
        float spacing = spacingAtFullDetail * scale;
        ...
        float topOfChestHeight = chest.Pose.Position.Y + chestShape.Radius + bodyRadius;
        ...
        float midpoint = (widthInBodies * 0.5f - 0.5f);
        float zRange = (chestShape.Radius * 0.65f) / spacing;
        float xRange = (chestShape.Radius * 0.5f + chestShape.HalfLength) / spacing;
        int minX = (int)MathF.Ceiling(midpoint - xRange);
        int maxX = (int)(midpoint + xRange);
        int minZ = (int)MathF.Ceiling(midpoint - zRange);
        int maxZ = (int)(midpoint + zRange);

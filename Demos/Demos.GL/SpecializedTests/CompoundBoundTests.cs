...
public class CompoundBoundTests : Demo
{
    ...
    void GetArcExpansion(Vector3 offset, Vector3 angularVelocity, float dt, out Vector3 minExpansion, out Vector3 maxExpansion)
    {
        ...
        Vector3 min = Vector3.Min(endpoint, offset);
        Vector3 max = Vector3.Max(endpoint, offset);
        minExpansion = minExpansion - min;
        maxExpansion = maxExpansion - max;
    }
    ...
    {
        GetArcExpansion(localPoseA, angularVelocityA, dt, out Vector3 minExpansionA, out Vector3 maxExpansionA);
        GetArcExpansion(-offsetB, -angularVelocityB, dt, out Vector3 minExpansionB, out Vector3 maxExpansionB);
    ...
    public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
    {
        ...
        {
            Sphere testShape = new(0);
            Quaternion orientationA = Quaternion.Identity;
            Quaternion orientationB = Quaternion.Identity;

            BodyVelocity velocityA = new(GetRandomVector(2, random), GetRandomVector(1, random));
            BodyVelocity velocityB = new(GetRandomVector(2, random), GetRandomVector(1, random));
            Vector3 offsetB = GetRandomVector(2, random);
            RigidPose localPoseA = new(GetRandomVector(2, random), Quaternion.Identity);
            float dt = 10f;

            const int pathPointCount = 512;
            Vector3[] localPathPoints = new Vector3[pathPointCount];
            ...
            for (int i = 0; i < pathPointCount; ++i)
            {
                ...
                PoseIntegration.Integrate(orientationB, velocityB.Angular, t, out Quaternion integratedB);
                Vector3 worldRotatedPoint = velocityA.Linear * t - velocityB.Linear * t - offsetB + QuaternionEx.Transform(localPoseA.Position, integratedA);
                localPathPoints[i] = QuaternionEx.Transform(worldRotatedPoint, QuaternionEx.Conjugate(integratedB));
            }
            Vector3 referenceSweep = localPathPoints[pathPointCount - 1] - localPathPoints[0];
            Vector3 sweepMin = Vector3.Min(localPathPoints[pathPointCount - 1], localPathPoints[0]);
            Vector3 sweepMax = Vector3.Max(localPathPoints[pathPointCount - 1], localPathPoints[0]);
            Vector3 referenceMin = new(float.MaxValue);
            Vector3 referenceMax = new(float.MinValue);
            for (int i = 0; i < pathPointCount; ++i)
            {
                referenceMin = Vector3.Min(referenceMin, localPathPoints[i]);
                referenceMax = Vector3.Max(referenceMax, localPathPoints[i]);
            }
            Vector3 referenceMinExpansion = referenceMin - sweepMin;
            Vector3 referenceMaxExpansion = referenceMax - sweepMax;

            TypedIndex shapeIndex = Simulation.Shapes.Add(testShape);
            BoundingBoxHelpers.GetLocalBoundingBoxForSweep(shapeIndex, Simulation.Shapes, localPoseA, orientationA, velocityA, offsetB, orientationB, velocityB, dt, out Vector3 naiveSweep, out Vector3 naiveMin, out Vector3 naiveMax);
            naiveMin += Vector3.Min(naiveSweep, default);
            naiveMax += Vector3.Max(naiveSweep, default);
            Simulation.Shapes.Remove(shapeIndex);
            BoundingBox.CreateMerged(naiveMin, naiveMax, referenceMin, referenceMax, out Vector3 combinedMin, out Vector3 combinedMax);
            ...
            Vector3 expansionOffset = new(0, 0, 65);

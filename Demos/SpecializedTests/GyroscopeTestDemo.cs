...
public class GyroscopeTestDemo : Demo
{
    public override void Initialize(ContentArchive content, Camera camera)
    {
        ...
        BodyHandle gyroBaseBody = Simulation.Bodies.Add(BodyDescription.CreateConvexKinematic(new Vector3(0, 2, 0), Simulation.Shapes, new Box(.1f, 4, .1f)));
        BodyHandle gyroSpinnerBody = Simulation.Bodies.Add(BodyDescription.CreateConvexDynamic(new Vector3(2, 4, 0), (default, new Vector3(300, 0, 0)), 1, Simulation.Shapes, new Box(0.1f, 1f, 1f)));
        ...
        builder.BuildDynamicCompound(out Buffer<CompoundChild> children, out BodyInertia inertia, out _);
        builder.Dispose();
        TypedIndex dzhanibekovShape = Simulation.Shapes.Add(new Compound(children));
        BodyHandle dzhanibekovSpinnerBody = Simulation.Bodies.Add(
            BodyDescription.CreateDynamic(new Vector3(6, 4, 0), (new Vector3(0, 0, 1), new Vector3(3, 1e-5f, 0)), inertia, dzhanibekovShape, 0.01f));
        BodyHandle dzhanibekovBaseBody = Simulation.Bodies.Add(BodyDescription.CreateConvexKinematic(new Vector3(6, 1, 0), Simulation.Shapes, new Box(.1f, 2, .1f)));

...
namespace BEPUPhysics.CarDrivingDemo
{
    ...
    public class DemoHost : IDisposable
    {
        ...
        public DemoHost(GameLoop loop, BEPUDemoControlsBindings? controls = null)
        {
            ...
            Assembly assembly = typeof(DemoHost).Assembly;
            using Stream stream = assembly.GetManifestResourceStream("BEPUPhysics.CarDrivingDemo.Contents.Carlito-Regular.font")
                ?? throw new FileNotFoundException("Embedded font resource was not found.");
            using BinaryReader reader = new(stream, Encoding.UTF8, true);
            FontContent fontContent = FontIO.Load(reader);

            font = new Font(fontContent);

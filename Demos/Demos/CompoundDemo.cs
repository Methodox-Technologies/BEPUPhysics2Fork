...
public class CompoundDemo : Demo
{
    public override void Initialize(ContentArchive content, Camera camera)
    {
        ...
        using (CompoundBuilder compoundBuilder = new(BufferPool, Simulation.Shapes, 8))
        {
            {
                ...
                Capsule capsuleChildShape = new(0.5f, 0.5f);
                RigidPose capsuleLocalPose = new() { Position = new Vector3(-0.5f, 4, 4), Orientation = Quaternion.Identity };
                Box boxChildShape = new(0.5f, 1f, 1.5f);
                RigidPose boxLocalPose = new() { Position = new Vector3(0.5f, 4, 4), Orientation = Quaternion.Identity };
                ...
                compoundBuilder.BuildDynamicCompound(out Buffer<CompoundChild> compoundChildren, out BodyInertia compoundInertia, out Vector3 compoundCenter);
            ...
            {
                Sphere gridShape = new(0.5f);
                const float gridSpacing = 1.5f;
                const int gridWidth = 3;
                TypedIndex gridShapeIndex = Simulation.Shapes.Add(gridShape);
                BodyInertia gridBoxInertia = gridShape.ComputeInertia(1);
                ...
                compoundBuilder.BuildDynamicCompound(out Buffer<CompoundChild> gridChildren, out BodyInertia gridInertia, out Vector3 center);
                compoundBuilder.Reset();
                Compound gridCompound = new(gridChildren);
                BodyDescription bodyDescription = BodyDescription.CreateDynamic(RigidPose.Identity, gridInertia, Simulation.Shapes.Add(gridCompound), 0.01f);
                for (int i = 0; i < 4; ++i)
                {
            ...
            //Build a table and use it for a couple of different tests. 
            {
                ...
                TypedIndex legShapeIndex = Simulation.Shapes.Add(legShape);
                ...
                compoundBuilder.Reset();
                Compound table = new(tableChildren);
                BodyDescription tableDescription = BodyDescription.CreateDynamic(RigidPose.Identity, tableInertia, Simulation.Shapes.Add(table), 0.01f);
                ...
                //Put a table on top of a sphere to stress out nonconvex reduction for divergent normals.
                {
                    ...
                    Sphere sphereShape = new(3);
                    TypedIndex sphereIndex = Simulation.Shapes.Add(sphereShape);
                    StaticDescription sphereDescription = new(new Vector3(10, 2, 0), sphereIndex);
                ...
                //Put another table on the ground, but with a clamp-ish thing on it that generates opposing normals.
                {
                    ...
                    Box clampPieceShape = new(2f, 0.1f, 0.3f);
                    BodyInertia clampPieceInverseInertia = clampPieceShape.ComputeInertia(1f);
                    TypedIndex clampPieceShapeIndex = Simulation.Shapes.Add(clampPieceShape);
                    RigidPose clamp0 = new() { Position = new Vector3(0, -0.2f, -1.1f), Orientation = Quaternion.Identity };
                    RigidPose clamp1 = new() { Position = new Vector3(0, 0.2f, -1.1f), Orientation = Quaternion.Identity };
                    RigidPose clamp2 = new() { Position = new Vector3(0, -0.2f, 0), Orientation = Quaternion.Identity };
                    RigidPose clamp3 = new() { Position = new Vector3(0, 0.2f, 0), Orientation = Quaternion.Identity };
                    RigidPose clamp4 = new() { Position = new Vector3(0, -0.2f, 1.1f), Orientation = Quaternion.Identity };
                    RigidPose clamp5 = new() { Position = new Vector3(0, 0.2f, 1.1f), Orientation = Quaternion.Identity };
                    ...
                    compoundBuilder.BuildDynamicCompound(out Buffer<CompoundChild> clampChildren, out BodyInertia clampInertia, out Vector3 clampCenter);
                    compoundBuilder.Reset();
                    Compound clamp = new(clampChildren);
                    BodyDescription clampDescription = BodyDescription.CreateDynamic(
            ...
            //Create a tree-accelerated big compound.
            {
                ...
                TypedIndex treeCompoundBoxShapeIndex = Simulation.Shapes.Add(treeCompoundBoxShape);
                ...
                TypedIndex compoundIndex = Simulation.Shapes.Add(compound);
                for (int i = 0; i < 8; ++i)
                {
        ...
        {
            Box boxShape = new(256, 1, 256);
            TypedIndex groundShapeIndex = Simulation.Shapes.Add(boxShape);
            StaticDescription groundDescription = new(RigidPose.Identity, groundShapeIndex);
